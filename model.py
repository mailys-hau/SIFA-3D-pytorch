...
from networks import SIFADis, SIFADisAux, SIFAGenerator, SIFAEncoder, SIFADecoder, SIFASeg
from net_utils import init_weights, get_scheduler, RandomElementPool
...
class SIFA(nn.Module):
    def __init__(self, params, mode="train"):
        ...
        self.enc = SIFAEncoder(dimensions=dim)
        self.dec = SIFADecoder(dimensions=dim, skip=self.skip)
        self.seg = SIFASeg(dimensions=dim, cout=num_classes)
        self.disA = SIFADisAux(dimensions=dim)
        self.disB = SIFADis(dimensions=dim)
        self.disSeg = SIFADis(dimensions=dim, cin=num_classes)
        #optimizer
        ...
        # fake image pool
        self.fakeA_pool = RandomElementPool()
        self.fakeB_pool = RandomElementPool()
        # lr update
        self.seg_opt_sch = get_scheduler(self.seg_opt)
        #loss
        self.segloss = DiceCeLoss(num_classes)
        #self.segloss = DiceFocalLoss(include_background=False, to_onehot_y=True, lambda_dice=0.7, lambda_focal=0.3)
    ...
    def update_seg(self,imagesa,imagesb,labelsa):
        ...
        seg_loss_B = self.segloss(self.pred_mask_fake_b, self.labelA) \
             + self.criterionL1(self.realA, self.cycleA) \
             + self.criterionL1(self.realB, self.cycleB) \
             + 0.1 * self.backward_G(self.disA, self.fakeA) \
             + 0.1 * self.backward_G(self.disSeg, self.pred_mask_b) \
             + 0.1 * self.backward_G(self.disA, self.fakeA, aux=True)
        seg_loss_B.backward()
        self.enc_opt.step()
        self.seg_opt.step()
    ...
    def sample_image(self, epoch, save_dir):
        sample_image_dir = save_dir.joinpath("sample_image")
        sample_image_dir.mkdir(parents=True, exist_ok=True)
        #print(sample_image_dir, epoch + 1)
        for i in range(self.realA.shape[0]):
            template = "{sample_image_dir}/{net}_epoch-{epoch + 1:03d}.nii.gz"
            nets = ["realA", "realB", "fakeA", "fakeB", "cycleA", "cycleB"]
            for net in nets:
