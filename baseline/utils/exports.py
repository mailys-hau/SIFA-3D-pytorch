...
def saveimage(batch_data, outputs, device, cf, args, epoch, test=False):
    ...
    if test:
        if hasattr(args, 'inference'):
            if not exists(join(args.pdir, cf.export_params.save_dir, args.job, f'best_outputs/f{args.fold}/inference')):
                os.makedirs(join(args.pdir, cf.export_params.save_dir, args.job, f'best_outputs/f{args.fold}/inference'))
            odir = join(args.pdir, cf.export_params.save_dir, args.job, f'best_outputs/f{args.fold}/inference')
        else:
    ...
    inputs_md = inputs.meta
    labels_md = labels.meta

    outputs = MetaTensor(outputs, meta=labels_md)
    ...
    if test:
        ...
        post_pred = Compose([EnsureType(), AsDiscrete(argmax=True),
                             KeepLargestConnectedComponent(applied_labels=l, num_components=1)
                             ])
